...
;(function () { // eslint-disable-line no-extra-semi
  ...
  */
  ...
  var Gun = (typeof window !== 'undefined' ? window : global).Gun;
    ...
    return new Promise(function (resolve, reject) {
      ...
      root.get('alias/' + alias).get(function (rat, rev) {
        ...
        Gun.obj.map(rat.put, function (at, pub) {
          if (!pub.slice || pub.slice(0, 4) !== 'pub/') { return }
          c++;
  ...
  function each (msg) { // TODO: Warning: Need to switch to `gun.on('node')`! Do not use `Gun.on('node'` in your apps!
    ...
    Gun.node.is(msg.put, function (val, key, node) {
      ...
      SEA.read(val, false).then(function (data) {
  ...
  function security (msg) {
    ...
    if (msg.get) {
      ...
      if (soul) { // for now, only allow direct IDs to be read.
        if (soul === 'alias') { // Allow reading the list of usernames/aliases in the system?
          return to.next(msg); // yes.
        } else
        if (soul.slice(0, 6) === 'alias/') { // Allow reading the list of public keys associated with an alias?
          return to.next(msg); // yes.
    ...
    if (msg.put) {
      ...
      each.way = function (val, key) {
        ...
        if (key === '_') { return } // ignore meta data
        if (soul === 'alias') {  // special case for shared system data, the list of aliases.
          each.alias(val, key, node, soul); return;
        }
        if (soul.slice(0, 6) === 'alias/') { // special case for shared system data, the list of public keys for an alias.
          each.pubs(val, key, node, soul); return;
        }
        if (soul.slice(0, 4) === 'pub/') { // special case, account data for a public key.
      ...
      each.pub = function (val, key, node, soul, pub, user) { // Example: {_:#pub/asdf, hello:SEA['world',fdsa]}
        ...
        SEA.read(val, pub).then(function (data) {
          ...
          if ((rel = Gun.val.rel.is(data)) && (tmp = rel.split('~')) && tmp.length === 2) {
      ...
      each.any = function (val, key, node, soul, user) {
        ...
        if (!user || !(user = user._) || !(user = user.sea)) {
          if (user = at.sea.own[soul]) {
            ...
            SEA.read(val, user, function (data) {
              var rel;
          ...
          check['any' + soul + key] = 1;
          if ((tmp = soul.split('~')) && tmp.length == 2) {
    ...
    var doIt = function (resolve, reject) {
      ...
      recallCryptoKey(p, s).then(function (aesKey) {
        subtle.decrypt({
          ...
          try {
            return ctUtf8.slice ? JSON.parse(ctUtf8) : ctUtf8;
  ...
  SEA.write = function (mm, p, cb) {
    var doIt = function (resolve, reject) {
      ...
      var m = mm;
      if (m && m.slice && m.slice(0, 4) === 'SEA[') { return resolve(m) }
      if (mm && mm.slice) {
        // Needs to remove previous signature envelope
        while (m.slice(0, 4) === 'SEA[') {
          try {
  ...
  SEA.read = function (m, p, cb) {
    var doIt = function (resolve, reject) {
      var d;
      if (!m) {
        if (p === false) { return resolve(m) }
        return resolve();
      }
      if (!m.slice || m.slice(0, 4) !== 'SEA[') {
        if (p === false) { return resolve(m) }
        return resolve();
      }
      m = m.slice(3);
      try {
        m = m.slice ? JSON.parse(m) : m;
      } catch (e) { return reject(e) }
      m = m || '';
      d = m[0];
      try { d = d.slice ? JSON.parse(d) : d } catch (e) {}
      if (p === false) { resolve(d) }
